...
def prepare_data_4_sp(labels, features, dim):
    ...
    for i in range(instance_cnt):
        m = len(features[i])
        for j in range(m):
            indices.append([i, features[i][j][0]])  # 特征编号
            values.append(features[i][j][1])  # 特征值
            values_2.append(features[i][j][1] * features[i][j][1])  # 特征值的平方
            feature_indices.append(features[i][j][0])  # 特征编号
...
def single_run(feature_cnt, field_cnt, params):
    ...
    params['train_file'] = params['train_file'].replace('.csv', '.pkl').replace('.txt', '.pkl').replace('.fm', '.pkl')
    params['test_file'] = params['test_file'].replace('.csv', '.pkl').replace('.txt', '.pkl').replace('.fm', '.pkl')
    ...
    _indices = tf.placeholder(tf.int64, shape=[None, 2], name='raw_indices')  # [第i行, 特征编号] [i, features[i][j][0]]
    _values = tf.placeholder(tf.float32, shape=[None], name='raw_values')  # features[i][j][1]
    _values2 = tf.placeholder(tf.float32, shape=[None], name='raw_values_square')  # features[i][j][1] * features[i][j][1]
    _shape = tf.placeholder(tf.int64, shape=[2], name='raw_shape')  # [instance_cnt, dim]

    _y = tf.placeholder(tf.float32, shape=[None, 1], name='Y')  # label
    _ind = tf.placeholder(tf.int64, shape=[None])  # 特征编号
    ...
    for eopch in range(n_epoch):
        ...
        # for labels, features in load_data_from_file_batching(params['train_file'], batch_size):
        #     training_input_in_sp = prepare_data_4_sp(labels, features, feature_cnt)
        ...
        if eopch % 1 == 0:
            ...
            auc = predict_te_file(preds, sess, params['test_file'], feature_cnt, _indices, _values, _shape, _y,
...
def predict_te_file(preds, sess, test_file, feature_cnt, _indices, _values, _shape, _y, _values2, _ind, epoch,
    ...
    # for labels, features in load_data_from_file_batching(test_file, batch_size):
    #     test_input_in_sp = prepare_data_4_sp(labels, features, feature_cnt)
...
def build_model(_indices, _values, _values2, _shape, _y, _ind, feature_cnt, field_cnt, params):
    ...
    w_linear = tf.Variable(tf.truncated_normal([feature_cnt, 1], stddev=init_value, mean=0),
                           # tf.random_uniform([feature_cnt, 1], minval=-0.05, maxval=0.05),
    ...
    w_fm = tf.Variable(tf.truncated_normal([field_cnt, feature_cnt, dim], stddev=init_value / math.sqrt(float(dim)), mean=0),
    ...
    if params['is_use_dnn_part']:
        ...
        for layer_size in layer_sizes:
            ...
            cur_b_nn_layer = tf.Variable(tf.truncated_normal([layer_size], stddev=init_value, mean=0),
                                         name='b_nn_layer' + str(
                                             layer_idx))  # tf.get_variable('b_nn_layer' + str(layer_idx), [layer_size], initializer=tf.constant_initializer(0.0))
...
def pre_build_data_cache_if_need(infile, feature_cnt, batch_size):
    outfile = infile.replace('.csv', '.pkl').replace('.txt', '.pkl').replace('.fm', '.pkl')
...
def run():
    ...
    params = {
        ...
        'train_file': '../../output/ffm/train.ffm',  # 'data/part01.svmlight_balanced.csv',
        'test_file': '../../output/ffm/test.ffm',  # 'data/part02.svmlight.csv',
