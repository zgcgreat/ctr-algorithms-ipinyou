...
def parse_args():
    ...
    parser.add_argument('--layers', nargs='?', default='[500, 500, 500]',
...
class NeuralFM(BaseEstimator, TransformerMixin):
    def __init__(self, field_cnt, features_M, hidden_factor, layers, loss_type, pretrain_flag, epoch, batch_size,
        ...
        self.nn_input_dim = self.field_cnt * self.hidden_factor + self.field_cnt
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            self.FM = tf.concat([w_fm_nn_input, nonzero_weights], axis=1)

            print('w_fm_nn_input.shape:', self.FM.shape)
            ...
            for i in range(0, len(self.layers)):
                ...
                self.nn_out = tf.matmul(self.FM, self.weights['prediction'])  # None * 1
            ...
            # #                                   1)  # None * 1
            # Bias = self.weights['bias'] * tf.ones_like(self.train_labels)  # None * 1
            # self.nn_out = tf.add_n([Bilinear, Bias])  # None * 1
            # self.nn_out = self.FM

            self.out = self.lin_out + self.fm_out + self.nn_out # self.lin_out + self.fm_out + self.nn_out
            ...
            if self.loss_type == 'square_loss':
                ...
                if self.lamda_bilinear > 0:
                    ...
                    #     self.lamda_bilinear)(self.weights['feature_embeddings'])  # regulizer
                    self.loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.out, labels=self.train_labels)) \
                         #         + tf.contrib.layers.l2_regularizer(
                         # self.lamda_bilinear)(self.weights['feature_embeddings'])
                    + 0.001 * tf.nn.l2_loss(self.weights['feature_embeddings'])
                    + 0.001 * tf.nn.l2_loss(self.weights['linear_weights'])
                    ...
                    for i in range(0, len(self.layers)):
                        self.loss += 0.0001 * tf.nn.l2_loss(self.weights['layer_%d' % i])
                else:
    ...
    def _initialize_weights(self):
        ...
        if self.pretrain_flag > 0:  # with pretrain
            ...
            all_weights['bias_0'] = tf.Variable(tf.random_uniform([1, self.layers[0]], minval=minval,
                                                                   maxval=maxval, dtype=tf.float32), dtype=tf.float32)  # 1 * layers[0]
            ...
            all_weights['stacking_weights'] = tf.Variable(tf.random_uniform([3, 1], 0.0, 0.01),
        ...
        if num_layer > 0:
            ...
            # all_weights['layer_0'] = tf.Variable(
            #     tf.random_normal([self.nn_input_dim, self.layers[0]], mean=0.0, stddev=1e-3, dtype=tf.float32),
            #     dtype=tf.float32)
...
if __name__ == '__main__':
    ...
    if args.verbose > 0:
        print(
            "DeepFM: dataset=%s, hidden_factor=%d, dropout_keep=%s, layers=%s, loss_type=%s, pretrain=%d, #epoch=%d, batch=%d, lr=%.4f, lambda=%.4f, optimizer=%s, batch_norm=%d, activation=%s, early_stop=%d"
