...
#################### Arguments ####################
def parse_args():
    parser = argparse.ArgumentParser(description="Run FNN.")
...
class NeuralFM(BaseEstimator, TransformerMixin):
    def __init__(self, field_cnt, features_M, hidden_factor, layers, loss_type, pretrain_flag, epoch, batch_size,
        ...
        self.nn_input_dim = self.field_cnt * self.hidden_factor  # self.field_cnt * self.hidden_factor  + self.hidden_factor
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            if self.loss_type == 'square_loss':
                ...
                if self.lamda_bilinear > 0:
                    ...
                    for i in range(0, len(self.layers)):
                        self.loss += 0.001 * tf.nn.l2_loss(self.weights['layer_%d' % i])
                else:
...
if __name__ == '__main__':
    ...
    if args.verbose > 0:
        print(
            "FNN: dataset=%s, hidden_factor=%d, dropout_keep=%s, layers=%s, loss_type=%s, pretrain=%d, #epoch=%d, batch=%d, lr=%.4f, lambda=%.4f, optimizer=%s, batch_norm=%d, activation=%s, early_stop=%d"
