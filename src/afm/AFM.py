...
class AFM(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            # Input data.
            self.train_features = tf.placeholder(tf.int64, shape=[None, None],
    ...
    def train(self, Train_data, Validation_data, Test_data):  # fit a dataset
        ...
        if self.verbose > 0:
            t2 = time()
            init_train, train_logloss, train_auc = self.evaluate(Train_data)
            init_valid, valid_logloss, valid_auc = self.evaluate(Validation_data)
        ...
        for epoch in range(self.epoch):
            ...
            train_result, train_logloss, train_auc = self.evaluate(Train_data)
            valid_result, valid_logloss, valid_auc = self.evaluate(Validation_data)
            self.train_rmse.append(train_logloss)
            self.valid_rmse.append(valid_logloss)

            if self.verbose > 0 and epoch % self.verbose == 0:
                # print("Epoch %d [%.1f s]\ttrain=%.4f, validation=%.4f [%.1f s]"
                #       % (epoch + 1, t2 - t1, train_result, valid_result, time() - t2))
                print("Epoch %d train_loss=%.8f\tvalid_loss=%.8f\ttrain_auc=%.8f\tvalid_auc=%.8f"
                  % (epoch + 1, train_logloss, valid_logloss, train_auc, valid_auc))
