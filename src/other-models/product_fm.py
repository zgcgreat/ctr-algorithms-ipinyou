...
#################### Arguments ####################
def parse_args():
    parser = argparse.ArgumentParser(description="Run ProductFM.")
    ...
    parser.add_argument('--layers', nargs='?', default='[900]',
                        help="Size of each layer.")
    parser.add_argument('--keep_prob', nargs='?', default='[1, 1, 1, 1, 1]',
...
class NeuralFM(BaseEstimator, TransformerMixin):
    def __init__(self, field_cnt, features_M, hidden_factor, layers, loss_type, pretrain_flag, epoch, batch_size,
        ...
        self.nn_input_dim = self.field_cnt +self.field_cnt * self.hidden_factor  # self.field_cnt * self.hidden_factor  + self.hidden_factor
        self.product_dim = self.num_pairs + self.nn_input_dim
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            # _________ linear part _____________
            nonzero_weights = tf.nn.embedding_lookup(self.weights['linear_weights'], self.train_features)
            self.lin_out = tf.reduce_sum(nonzero_weights, 1)
            self.lin_out = tf.reshape(self.lin_out, [-1, 1])
            ...
            self.fm_out = tf.add_n([Bilinear, Bias])  # None * 1
            ...
            self.FM = tf.concat([w_fm_nn_input, ip], 1)
            print('product_nn_input.shape:', self.FM.shape)
            for i in range(0, len(self.layers)):
                self.FM = tf.add(tf.matmul(self.FM, self.weights['product_layer_%d' % i]),
            ...
            # self.out = tf.concat([deep_out, product_out], 1)
            self.out = product_out

            print('out shape', self.out.shape)
            self.out = tf.matmul(self.out, self.weights['prediction'])  # None * 1

            self.out = (self.lin_out + self.fm_out + self.out) / 1
            ...
            if self.loss_type == 'square_loss':
                ...
                if self.lamda_bilinear > 0:
                    ...
                    for i in range(0, len(self.layers)):
                        # self.loss += 0.001 * tf.nn.l2_loss(self.weights['deep_layer_%d' % i])
                        self.loss += 0.001 * tf.nn.l2_loss(self.weights['product_layer_%d' % i])
    ...
    def _initialize_weights(self):
        ...
        if num_layer > 0:
            ...
            #     np.random.normal(loc=0, scale=glorot, size=(self.nn_input_dim, self.layers[0])), dtype=np.float32)
            ...
            all_weights['deep_layer_0'] = tf.Variable(
                tf.random_uniform([self.nn_input_dim, self.layers[0]], minval=minval,
                                  maxval=maxval, dtype=tf.float32), dtype=tf.float32)
            all_weights['deep_bias_0'] = tf.Variable(tf.random_uniform([1, self.layers[0]], minval=minval,
                                                                       maxval=maxval, dtype=tf.float32),
                                                     dtype=tf.float32)  # 1 * layers[0]
            all_weights['product_layer_0'] = tf.Variable(
                tf.random_uniform([self.product_dim, self.layers[0]], minval=minval,
                                  maxval=maxval, dtype=tf.float32), dtype=tf.float32)
            all_weights['product_bias_0'] = tf.Variable(tf.random_uniform([1, self.layers[0]], minval=minval,
                                                                          maxval=maxval, dtype=tf.float32),
                                                        dtype=tf.float32)

            for i in range(1, num_layer):
                glorot = np.sqrt(6.0 / (self.layers[i - 1] + self.layers[i]))
                ...
                all_weights['product_layer_%d' % i] = tf.Variable(
                    np.random.normal(loc=0, scale=glorot, size=(self.layers[i - 1], self.layers[i])),
                    dtype=np.float32)  # layers[i-1]*layers[i]
                all_weights['product_bias_%d' % i] = tf.Variable(
                    np.random.normal(loc=0, scale=glorot, size=(1, self.layers[i])), dtype=np.float32)  # 1 * layer[i]
            # prediction layer
    ...
    def eva_termination(self, valid):
        if self.loss_type == 'square_loss':
            if len(valid) > 4:
                if valid[-1] > valid[-2] and valid[-2] > valid[-3] and valid[-3] > valid[-4]:
                    return True
        else:
            if len(valid) > 4:
                if valid[-1] > valid[-2] and valid[-2] > valid[-3] and valid[-3] > valid[-4]:
...
if __name__ == '__main__':
    ...
    if args.verbose > 0:
        print(
            "ProductFM: dataset=%s, hidden_factor=%d, dropout_keep=%s, layers=%s, loss_type=%s, pretrain=%d, #epoch=%d, batch=%d, lr=%.4f, lambda=%.4f, optimizer=%s, batch_norm=%d, activation=%s, early_stop=%d"
